...
```
Monitor:
  description: "PR watch: PR #123 CI"
  persistent: true
  command: |
    while true; do
      checks="$(gh pr checks 123 --json name,bucket 2>/dev/null || echo '[]')"
...
If the same check has been pending more than **30 minutes** without a state change, the Monitor must emit a `[stalled]` event and you should **ask the user** whether to keep waiting. (The cross-platform test matrix is large - Node 18/20/24 x Java 8/11/17/21/25 x ubuntu/macos/windows - and the Windows runner can spend several minutes letting `njre` download a JRE; MegaLinter adds a few more. Don't panic early.) Do not poll faster than 5 minutes.

### 5. Fix the failures (delegate to `pr-fix`)

Spawn the `pr-fix` agent (Opus) via the Agent tool. Pass it the branch, PR number, current HEAD SHA, and the failure list from `pr-watch` (job names, error types, key log lines). `pr-fix` owns the diagnosis, the fix, local validation (`npm run test` / `npm run lint:fix`), and the commit + push (including all the git-safety rules).
...
```
Cycle 2: Test (windows-latest) failed (assertion in test/java-caller.test.js:42), pushed e0a44f1. Waiting 5m.
```
...
- A plain `git push` is the norm here: this repo's MegaLinter does not auto-commit fixes (no `APPLY_FIXES`), so there are no bot commits to reconcile. Any force-push needs explicit user permission - if `pr-fix` reports it needs one, ask the user.
- If `gh` is not authenticated or the repo isn't a GitHub repo, **STOP** and tell the user.
- Use **npm** not yarn (`package-lock.json` and CI's `npm ci` are the source of truth), never weaken or skip tests to force green. `pr-fix` enforces these; if it reports a violation it could not avoid, stop and ask.
